...
function getRegionalSalesStatus(value) {
  if (value >= 100) return 'bg-emerald-500/15 text-emerald-300'
  if (value >= 95) return 'bg-amber-500/15 text-amber-300'
    ...
    <section className="mx-auto mt-8 max-w-7xl rounded-2xl border border-slate-800 bg-slate-900/80 p-4 md:p-6">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-300">Auto-generated executive summary based on current store performance data.</p>
        </div>
        <button
          type="button"
                ...
                {report.regionalBreakdown.map((region) => (
                  <tr key={region.region} className="border-t border-slate-800 text-slate-200">
                    ...
                    <td className="px-3 py-2">
                      <span className={`rounded-md px-2 py-1 text-xs font-semibold ${getRegionalSalesStatus(region.salesVsTarget)}`}>
                        {formatPercent(region.salesVsTarget)}
                      </span>
                    </td>
